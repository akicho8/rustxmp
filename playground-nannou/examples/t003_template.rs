...
impl MyPoint {
    ...
    fn update(&mut self, app: &App, model: &mut Model) {
        let mouse = vec2(app.mouse.x, app.mouse.y);
        let distance = self.position.distance(mouse);
        self.hit_p = distance < 50.0;
        if self.hit_p {
            if model.mouse_down {
                self.position = mouse
            }
...
impl Ball {
    fn new() -> Self {
        Ball {
            my_pt1: MyPoint::new(vec2(0.0, 0.0)),
            my_pt2: MyPoint::new(vec2(0.0, 0.0)),
        }
    }

    fn update2(&mut self, _app: &App, _model: &mut Model) {
        // self.my_pt1.update(&app, &model);
        // self.my_pt2.update(&app, &model);
    }

    fn display(&self, draw: &Draw) {
        draw.arrow()
            .points(self.my_pt1.position, self.my_pt2.position)
            .color(DODGERBLUE);
    }
}
...
fn update(app: &App, model: &mut Model, _update: Update) {
    model.ball.update2(&app, &mut model);
