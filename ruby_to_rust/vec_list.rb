...
b  # =>
  ...
  {
    :ruby_method => "v[b], v[a] = v[a], v[b]",
    :rust_method => "swap(a, b)",
    :ruby_example => <<~EOT,
v = [5, 6, 7, 8]
v[2], v[1] = v[1], v[2]
v  # =>
...
[5, 6, 0, 7, 8, 0, 9].reverse.chunk { |e| e == 0 ? nil : true }.map { |e| e.last.reverse } # =>
  ...
  {
    :ruby_method => "String#split(?, n) の配列版風",
...
"5607809".reverse.split("0", 2).collect(&:reverse)  # =>
  ...
  {
    :ruby_method => "String#start_with? の配列版",
...
[5, 6, 7].starts_with(&[5, 6])  // =>
  ...
  {
    :ruby_method => "String#end_with? の配列版",
...
"567".end_with?("67")                    # =>
  ...
  {
    :ruby_method => "String#delete_prefix の配列版",
...
[5, 6, 7, 8].strip_prefix(&[5, 6])  // =>
  ...
  {
    :ruby_method => "String#delete_suffix の配列版",
...
v  # =>
  ...
  EOT
    :rust_feature => nil,
    :desc => "順番を気にしないのであれば O(1) で消せるのが特徴",
...
v  # =>
  ...
  EOT
    :rust_feature => nil,
    :desc => "値を参照するたびにクロージャを呼んでいるっぽい。使いどころが難しい。",
...
[5, 7, 9].binary_search(&7)  // =>
  EOT
    :rust_feature => nil,
    :desc => "bsearch とは使い勝手がぜんぜん違う。ソート済み配列だけに使える contains の速い版だろうか。",
