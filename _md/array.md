...
| each_cons 余りは捨てる                | iter.tuples                 | [DOC](https://docs.rs/itertools/latest/itertools/trait.Itertools.html#tuples) [実行](https://play.rust-lang.org/?code=fn+main%28%29+%7B%0A++++use+itertools%3A%3AItertools%3B%0A++++println%21%28%0A++++++++%22%7B%3A%3F%7D%22%2C%0A++++++++%5B5%2C+6%2C+7%2C+8%5D.iter%28%29.tuples%3A%3A%3C%28_%2C+_%29%3E%28%29.collect%3A%3A%3CVec%3C_%3E%3E%28%29%0A++++%29%3B%0A%7D%0A&version=nightly&edition=2021)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
```ruby:Ruby
v = [5, 6, 7]
v.find { |e| e == 6 } || v.first # => 6
v.find { |e| e == 0 } || v.first # => 5
```
```rust:Rust
use itertools::Itertools;
...
```ruby:Ruby
v = [5, 6, 7]
v.find { |e| e == 6 } || v.last # => 6
v.find { |e| e == 0 } || v.last # => 7
```
```rust:Rust
use itertools::Itertools;
...
```rust:Rust
use itertools::Itertools;
[5, 6, 7, 8].iter().tuples::<(_, _)>().collect::<Vec<_>>() // => [(5, 6), (7, 8)]
```
[DOC](https://docs.rs/itertools/latest/itertools/trait.Itertools.html#tuples) [実行](https://play.rust-lang.org/?code=fn+main%28%29+%7B%0A++++use+itertools%3A%3AItertools%3B%0A++++println%21%28%0A++++++++%22%7B%3A%3F%7D%22%2C%0A++++++++%5B5%2C+6%2C+7%2C+8%5D.iter%28%29.tuples%3A%3A%3C%28_%2C+_%29%3E%28%29.collect%3A%3A%3CVec%3C_%3E%3E%28%29%0A++++%29%3B%0A%7D%0A&version=nightly&edition=2021)
...
zip.([100, 200], [5, 6, 7, 8]).to_a  # => [100, 5, 200, 6]
zip.([5, 6, 7, 8], [100, 200]).to_a  # => [5, 100, 6, 200]
```
```rust:Rust
[100, 200].iter().zip([5, 6, 7, 8].iter()).collect::<Vec<_>>() // => [(100, 5), (200, 6)]
[5, 6, 7, 8].iter().zip([100, 200].iter()).collect::<Vec<_>>() // => [(5, 100), (6, 200)]
```
ペアになれなかった要素は無視されるので注意しよう。また引数は1つしか与えられない。
[DOC](https://doc.rust-lang.org/std/iter/trait.Iterator.html#method.zip) [実行](https://play.rust-lang.org/?code=fn+main%28%29+%7B%0A++++println%21%28%0A++++++++%22%7B%3A%3F%7D%22%2C%0A++++++++%5B100%2C+200%5D%0A++++++++++++.iter%28%29%0A++++++++++++.zip%28%5B5%2C+6%2C+7%2C+8%5D.iter%28%29%29%0A++++++++++++.collect%3A%3A%3CVec%3C_%3E%3E%28%29%0A++++%29%3B%0A++++println%21%28%0A++++++++%22%7B%3A%3F%7D%22%2C%0A++++++++%5B5%2C+6%2C+7%2C+8%5D%0A++++++++++++.iter%28%29%0A++++++++++++.zip%28%5B100%2C+200%5D.iter%28%29%29%0A++++++++++++.collect%3A%3A%3CVec%3C_%3E%3E%28%29%0A++++%29%3B%0A%7D%0A&version=nightly&edition=2021)

## `zip の捨てない＆詰める版` → `iter.interleave`
```ruby:Ruby
interleave = -> a, b {
  enums = [a, b].collect(&:to_enum)
  Enumerator.new do |y|
...
interleave.([5, 6, 7, 8], [100, 200]).to_a  # => [5, 100, 6, 200, 7, 8]
interleave.([100, 200], [5, 6, 7, 8]).to_a  # => [100, 5, 200, 6, 7, 8]
```
```rust:Rust
use itertools::Itertools;
...
## `zip の捨てない＆詰める + EitherOrBoth` → `iter.zip_longest`
```ruby:Ruby
zip_longest = -> a, b {
  a = a.to_enum
  b = b.to_enum
...
## `zip の次が無いと終わり版` → `iter.interleave_shortest`
```ruby:Ruby
